...
RSpec.describe Order, type: :model do
  ...
  it "é inválido sem endereço de coleta" do
    ...
    order.valid?
    expect(order.errors[:pickup_address]).to include("Não pode estar vazio")
  end

  it "é inválido sem valor estimado" do
    order = Order.new(user: user, estimated_price: nil)
    order.valid?
    expect(order.errors[:estimated_price]).to include("Não pode estar vazio")
  end

  it "é inválido com valor estimado negativo" do
    order = Order.new(user: user, estimated_price: -10)
    order.valid?
    expect(order.errors[:estimated_price]).to include("must be greater than 0")
  end
end
